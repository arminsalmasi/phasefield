...
%     laplaceMu = x*0;
    ...
    %dG = G1const(c) - G0const(c);c(step, 1:N) = phi(step, 1:N) .* C0 + (1-phi(step, 1:N)) .* C1;dG = G1(c(step, 1:N)) - G0(c(step, 1:N));
       %Armin: from the text (G1-G0)
    %setting the chemical potential
    mu(step, 1:N) =  mu1(c(step, 1:N)) - mu0(c(step, 1:N)); %3e5 - 8e4 * c(step, 1:N); %mu = dg/dc
